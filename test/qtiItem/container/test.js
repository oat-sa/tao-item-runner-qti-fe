 ...
 **/
define(['taoQtiItem/qtiItem/core/feedbacks/ModalFeedback', 'taoQtiItem/qtiItem/helper/container'], function(
    ModalFeedback,
    container
) {
    'use strict';
    ...
    QUnit.test('set/getEncodedData', function(assert) {
        ...
        assert.equal(
            fb.body(),
            '<div class="x-tao-wrapper x-tao-customData1-customValueB x-tao-customData2-customValueC"><p>AAA</p></div>'
        );
        assert.equal(container.hasEncodedData(fb, 'customData1', 'customValueB'), true);
        assert.equal(container.getEncodedData(fb, 'customData1'), 'customValueB');
