...
define([
    ...
    QUnit.test('Strip prefixes', function(assert) {
        ...
        while (i--) {
            assert.equal(
                prefixed[i].prefix,
                'originalPrefix',
                'Initially this <' + prefixed[i].localName + '> element has an HTML5 prefix'
            );
        }
        ...
        while (i--) {
            assert.equal(
                prefixed[i].prefix,
                null,
                'After stripNs() this <' + prefixed[i].localName + '> element has no longer an HTML5 prefix'
            );
        }
    });
    ...
    QUnit.test('Restore prefixes', function(assert) {
        ...
        while (i--) {
            assert.equal(
                prefixed[i].prefix,
                'originalPrefix',
                '<' + prefixed[i].localName + '> element has the prefix "originalPrefix"'
            );
        }
    });
    ...
    QUnit.test('Add required namespace and prefixes', function(assert) {
        ...
        i = prefixed.length;
        while (i--) {
            assert.equal(prefixed[i].prefix, 'qh5', '<' + prefixed[i].localName + '> element has the prefix "qh5"');
        }
    });
});
