...
define([
    ...
    QUnit.cases
        ...
        .test('Simple elements parsing: ', function(data, assert) {
            ...
            assert.equal(
                serial.indexOf(data.expectedElement),
                0,
                'element is of the expected type, with serial ' + bodyElementsSerials[0]
            );
            assert.deepEqual(
                body.elements[serial].attributes,
                data.expectedAttributes,
                'element has the expected attributes'
            );
        });
        ...
        .init([
            {
                ...
                xml:
                    ...
                    '. How cool is that???</div>',
                expectedBody:
                    'this is a tooltip: <span data-role="tooltip-target" aria-describedby="_tooltip-63etvf7pktf2jb16d2a09y">my <strong>Target</strong></span>. How cool is that???'
            },
            {
