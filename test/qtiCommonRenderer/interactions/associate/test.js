define([
    ...
    QUnit.test('renders correctly', function (assert) {
        ...
        runner = qtiItemRunner('qti', associateData)
            .on('render', function () {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-associateInteraction').length,
                    1,
                    'the container contains a associate interaction .qti-associateInteraction'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-prompt-container').length,
                    1,
                    'the interaction contains a prompt'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .instruction-container').length,
                    1,
                    'the interaction contains a instruction box'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .choice-area').length,
                    1,
                    'the interaction contains a choice list'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice').length,
                    6,
                    'the interaction has 6 choices'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .result-area').length,
                    1,
                    'the interaction has a result area'
                );
                ...
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(1)').data('identifier'),
                    'A',
                    'the 1st choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(2)').data('identifier'),
                    'C',
                    'the 2nd choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(3)').data('identifier'),
                    'D',
                    'the 3rd choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(4)').data('identifier'),
                    'L',
                    'the 4th choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(5)').data('identifier'),
                    'M',
                    'the 5th choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice:nth-child(6)').data('identifier'),
                    'P',
                    'the 6th choice has the right identifier'
                );

                assert.equal(
                    $container.find('.qti-associateInteraction .result-area').children().length,
                    3,
                    'the interaction has 3 pairs area according to maxAssocation'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .result-area .target').length,
                    6,
                    'the interaction has 6 target box according to maxAssocation (3 pairs)'
                );
        ...
        runner = qtiItemRunner('qti', associateData)
            .on('render', function () {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-associateInteraction').length,
                    1,
                    'the container contains an associate interaction .qti-associateInteraction'
                );
    ...
    QUnit.test('enables to use a choice multiple times', function (assert) {
        ...
        pChoiceMatchMax =
            associateData.body.elements.interaction_associateinteraction_54787e6dad70d437146538.choices
                .choice_simpleassociablechoice_54787e6dadcdd949770698.attributes.matchMax;
                                        ...
                                        interactUtils.tapOn(
                                            ...
                                            function () {
                                                ...
                                                assert.equal(
                                                    $target2.text().trim(),
                                                    'Prospero',
                                                    'the target contains the choice text'
                                                );
                                                assert.ok(
                                                    $prospero.hasClass('deactivated'),
                                                    'the P choice is now deactivated'
                                                );
                        ...
                        interactUtils.tapOn(
                            ...
                            function () {
                                ...
                                assert.equal(
                                    $antonio.innerText,
                                    $target1.innerText,
                                    'Antonio has been added to the result area'
                                );
                                        ...
                                        interactUtils.tapOn(
                                            ...
                                            function () {
                                                assert.equal(
                                                    $capulet.innerText,
                                                    $target1.innerText,
                                                    'Capulet has replaced Antonio in the result area'
                                                );
                                                assert.ok(
                                                    !$antonio.hasClass('deactivated'),
                                                    'Antonio is not deactivated anymore'
                                                );
                                                assert.ok(
                                                    $capulet.hasClass('deactivated'),
                                                    'Capulet is now deactivated'
                                                );
            ...
            .on('statechange', function (state) {
                ...
                if (stateChangeCounter === 1) {
                    ...
                    assert.deepEqual(
                        ...
                        'The second pair is in the response'
                    );
                                    ...
                                    function () {
                                        interactUtils.tapOn(
                                            ...
                                            function () {
                                                ...
                                                assert.ok(
                                                    $antonio.hasClass('deactivated'),
                                                    'the A choice is deactivated'
                                                );
                                                assert.ok(
                                                    $capulet.hasClass('deactivated'),
                                                    'the C choice is deactivated'
                                                );
                                                assert.ok($target1.hasClass('filled'), 'the target is filled');
                                                assert.ok($target2.hasClass('filled'), 'the target is filled');
                                                ...
                                                _.delay(function () {
                                                    assert.ok(
                                                        !$antonio.hasClass('deactivated'),
                                                        'the A choice is not deactivated anymore'
                                                    );
                                                    assert.ok(
                                                        !$capulet.hasClass('deactivated'),
                                                        'the C choice is not deactivated anymore'
                                                    );
                                                    assert.ok(
                                                        !$target1.hasClass('filled'),
                                                        'the target is not filled anymore'
                                                    );
                                                    assert.ok(
                                                        !$target2.hasClass('filled'),
                                                        'the target is not filled anymore'
                                                    );
        ...
        runner = qtiItemRunner('qti', shuffled)
            .on('render', function () {
                assert.equal(
                    $container.find('.qti-interaction.qti-associateInteraction').length,
                    1,
                    'the container contains a choice interaction .qti-associateInteraction'
                );
                assert.equal(
                    $container.find('.qti-associateInteraction .qti-choice').length,
                    6,
