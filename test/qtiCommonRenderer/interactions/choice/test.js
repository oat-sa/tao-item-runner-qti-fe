    ...
    QUnit.test('renders correclty', function(assert) {
        ...
        runner = qtiItemRunner('qti', choiceData)
            .on('render', function() {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-choiceInteraction').length,
                    1,
                    'the container contains a choice interaction .qti-choiceInteraction'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-prompt-container').length,
                    1,
                    'the interaction contains a prompt'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .instruction-container').length,
                    1,
                    'the interaction contains a instruction box'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .choice-area').length,
                    1,
                    'the interaction contains a choice list'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice').length,
                    5,
                ...
                assert.equal(
                    ...
                    'space-shuttle-30-years-of-adventure',
                    'the .qti-item node has the right identifier'
                );

                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice:nth-child(1)').data('identifier'),
                    'Discovery',
                    'the 1st choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice:nth-child(2)').data('identifier'),
                    'Challenger',
                    'the 2nd choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice:nth-child(3)').data('identifier'),
                    'Pathfinder',
                    'the 3rd choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice:nth-child(4)').data('identifier'),
                    'Atlantis',
                    'the 4th choice has the right identifier'
                );
                assert.equal(
                    $container.find('.qti-choiceInteraction .qti-choice:nth-child(5)').data('identifier'),
                    'Endeavour',
        ...
        runner = qtiItemRunner('qti', choiceData)
            ...
            .on('statechange', function(state) {
                ...
                assert.deepEqual(
                    ...
                    'The discovery response is selected'
                );
                ready();
            })
            .init()
        ...
        runner = qtiItemRunner('qti', multipleChoiceData)
            .on('render', function() {
                ...
                assert.equal(
                    $container.find('.qti-choiceInteraction .instruction-container').length,
                    1,
                    'the interaction contains an instruction box'
                );
                assert.equal(
            ...
            .on('statechange', function(state) {
                if (++changes === 2) {
                    ...
                    assert.deepEqual(
                        state.RESPONSE,
                        { response: { list: { identifier: ['Discovery', 'Challenger'] } } },
                        'Discovery AND Challenger are selected'
                    );
                    ready();
                }
            })
        ...
        runner = qtiItemRunner('qti', shuffled)
            .on('render', function() {
                ...
                _.delay(function() {
                    ...
                    assert.deepEqual(
                        self.getState().RESPONSE.eliminated,
                        ['Discovery', 'Atlantis'],
                        'state is correct'
                    );
                    ready();
    ...
    QUnit.test('check dashes and dots in the identifier', function(assert) {
        ...
        runner = qtiItemRunner('qti', badIdentChoiceData)
            ...
            .on('statechange', function(state) {
                ...
                assert.deepEqual(
                    state.RESPONSE,
                    { response: { base: { identifier: 'Discovery-new.dot' } } },
                    'The Discovery-new.dot response is selected'
                );
