define([
    ...
    QUnit.test('video renders correctly', function(assert) {
        ...
        $container.one('playerrendered', function() {
            ...
            assert.equal(
                $container.find('.qti-interaction.qti-mediaInteraction').length,
                1,
                'the container contains a choice interaction .qti-mediaInteraction'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction .qti-prompt-container').length,
                1,
                'the interaction contains a prompt'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction .instruction-container').length,
                1,
                'the interaction contains a instruction box'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction video').length,
                1,
                'the interaction contains a video tag'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction video source').length,
                1,
                'the interaction contains a video source tag'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction video source').attr('src'),
                videoSampleUrl,
                'the interaction has proper file attached'
            );
    ...
    QUnit.test('audio renders correctly', function(assert) {
        ...
        $container.one('playerrendered', function() {
            ...
            assert.equal(
                $container.find('.qti-mediaInteraction audio').length,
                1,
                'the interaction contains an audio tag'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction audio source').length,
                1,
                'the interaction contains an audio source tag'
            );
            assert.equal(
                $container.find('.qti-mediaInteraction audio source').attr('src'),
                audioSampleUrl,
                'the interaction has proper file attached'
            );
            assert.equal(
            ...
            assert.equal(
                $container.find('.qti-mediaInteraction .control [data-control=play]').length,
                1,
                'the interaction has a play button'
            );
    ...
    if (!isHeadless) {
        QUnit.test('set state', function(assert) {
            ...
            assert.equal(
                $container.find('.qti-mediaInteraction').length,
                0,
