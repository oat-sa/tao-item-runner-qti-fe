    ...
    var strategies = [
        {
            ...
            handle: function defaultStrategy(url) {
                if (/assets/.test(url.toString())) {
                    return '/test/qtiCommonRenderer/interactions/graphicGapMatch/' + url.toString();
                }
                return url.toString();
            }
        }
    ...
    QUnit.test('renders correctly', function (assert) {
        ...
        runner = qtiItemRunner('qti', gapMatchData)
            .on('render', function () {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-graphicGapMatchInteraction').length,
                    1,
                    'the container contains a choice interaction .qti-graphicGapMatchInteraction'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .qti-prompt-container').length,
                    1,
                    'the interaction contains a prompt'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .instruction-container').length,
                    1,
                    'the interaction contains a instruction box'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .block-listing').length,
                    1,
                    'the interaction contains a choice area'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .block-listing .qti-choice').length,
                    6,
                    'the interaction has 6 choices'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .main-image-box').length,
                    1,
                    'the interaction contains a image'
                );
                assert.equal(
                    $container.find('.qti-graphicGapMatchInteraction .main-image-box rect').length,
                    6,
                ...
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        .eq(0)
                        .data('identifier'),
                    'gapimg_1',
                    'the 1st block has the right identifier'
                );
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        ...
                        .data('identifier'),
                    'gapimg_2',
                    'the 2nd block has the right identifier'
                );
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        ...
                        .data('identifier'),
                    'gapimg_3',
                    'the 3rd block has the right identifier'
                );
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        ...
                        .data('identifier'),
                    'gapimg_4',
                    'the 4th block has the right identifier'
                );
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        ...
                        .data('identifier'),
                    'gapimg_5',
                    'the 5th block has the right identifier'
                );
                assert.equal(
                    $container
                        .find('.qti-graphicGapMatchInteraction .block-listing .qti-choice')
                        ...
                        .data('identifier'),
                    'gapimg_6',
                    'the 6th block has the right identifier'
                );
    ...
    QUnit.test('enables to activate a gap filler', function (assert) {
        ...
        runner = qtiItemRunner('qti', gapMatchData)
            .on('render', function () {
                ...
                assert.ok($hotspot.get(0).classList.contains('basic'), 'The hotspot is not highlighted');

                interactUtils.tapOn(
                    $gapFiller,
                    function () {
                        assert.ok($gapFiller.hasClass('active'), 'The gap filler is now active');
                        assert.ok($hotspot.get(0).classList.contains('selectable'), 'The hotspot is now highlighted');

                        ready();
                    },
                    200
                ); // Safety delay for the border color to start changing, could be shortened by removing the animation
            })
            .assets(strategies)
            .init()
            .render($container);
                    ...
                    assert.deepEqual(
                        state.RESPONSE,
                        {
                            response: {
                                list: {
                                    directedPair: [
                                        ['gapimg_1', 'associablehotspot_6'],
                                        ['gapimg_3', 'associablehotspot_6']
                                    ]
                                }
                            }
                        },
        ...
        runner = qtiItemRunner('qti', gapMatchData)
            ...
            .on('statechange', function (state) {
                ...
                if (stateChangeCounter === 1) {
                    ...
                    assert.equal($container.find('.placed-fillers .qti-gapImg').length, 0, 'there are no filled gaps');
                    ready();
                }
            })
            .assets(strategies)
    ...
    QUnit.test('set the default response', function (assert) {
        ...
        runner = qtiItemRunner('qti', gapMatchData)
            .on('render', function () {
                ...
                this.setState({
                    RESPONSE: { response: { list: { directedPair: [['gapimg_1', 'associablehotspot_6']] } } }
                });
