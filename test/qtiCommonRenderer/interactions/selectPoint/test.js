...
define([
    ...
    var strategies = [
        {
            ...
            handle: function defaultStrategy(url) {
                if (/assets/.test(url.toString())) {
                    return '/test/qtiCommonRenderer/interactions/selectPoint/' + url.toString();
                }
                return url.toString();
            }
        }
    ...
    QUnit.cases
        ...
        .test('expose method', function(data, assert) {
            assert.equal(
                typeof selectPointRenderer[data.name],
                'function',
                'The selectPointRenderer exposes the method ' + data.name
            );
        });
    ...
    QUnit.test('renders correctly', function(assert) {
        ...
        runner = qtiItemRunner('qti', selectPointData)
            .on('render', function() {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-selectPointInteraction').length,
                    1,
                    'the container contains a choice interaction .qti-selectPointInteraction'
                );
                assert.equal(
                    $container.find('.qti-selectPointInteraction .qti-prompt-container').length,
                    1,
                    'the interaction contains a prompt'
                );
                assert.equal(
                    $container.find('.qti-selectPointInteraction .instruction-container').length,
                    1,
                    'the interaction contains a instruction box'
                );
                assert.equal(
                    $container.find('.qti-selectPointInteraction .main-image-box').length,
                    1,
