    ...
    QUnit.test('renders correclty', function(assert) {
        ...
        runner = qtiItemRunner('qti', matchData)
            .on('render', function() {
                ...
                assert.equal(
                    $container.find('.qti-interaction.qti-matchInteraction').length,
                    1,
                    'the container contains a choice interaction .qti-matchInteraction'
                );
                assert.equal(
                    $container.find('.qti-matchInteraction .qti-prompt-container').length,
                    1,
                    'the interaction contains a prompt'
                );
                assert.equal(
                    $container.find('.qti-matchInteraction .instruction-container').length,
                    1,
                    'the interaction contains a instruction box'
                );
                assert.equal(
                    $container.find('.qti-matchInteraction .match-interaction-area').length,
                    1,
                    'the interaction contains an interaction area'
                );
                assert.equal(
                    $container.find('.qti-matchInteraction .match-interaction-area table').length,
                    1,
                    'the interaction contains a table element'
                );
                assert.equal(
                    $container.find('.qti-matchInteraction .qti-choice').length,
                    7,
                ...
                assert.equal(
                    $container
                        .find('.qti-matchInteraction thead .qti-choice')
                        .eq(0)
                        .data('identifier'),
                    'C',
                ...
                assert.equal(
                    $container
                        .find('.qti-matchInteraction tbody .qti-choice')
                        .eq(0)
                        .data('identifier'),
                    'R',
                    'the 1st choice of the 1st match group has the right identifier'
    ...
    QUnit.test('enables to select a choice', function(assert) {
        ...
        runner = qtiItemRunner('qti', matchData)
            ...
            .on('statechange', function(state) {
                ...
                assert.deepEqual(
                    state.RESPONSE,
                    { response: { list: { directedPair: [['C', 'R']] } } },
                    'The pair CR is selected'
                );
    ...
    QUnit.test('enables to select multiple choices', function(assert) {
        ...
        runner = qtiItemRunner('qti', matchData)
            ...
            .on('statechange', function(state) {
                ...
                assert.deepEqual(
                    state.RESPONSE,
                    { response: { list: { directedPair: [['C', 'R'], ['D', 'T']] } } },
                    'The pair CR is selected'
                );
                ready();
            })
