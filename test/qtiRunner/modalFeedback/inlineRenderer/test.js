        ...
        new QtiLoader().loadItemData(itemData, function (_item) {
            ...
            renderer.load(function () {
                ...
                count = inlineRenderer.showFeedbacks(item, loader, renderer, testCase.itemSession, _.noop, function () {
                    ...
                    assert.equal(
                        $('.qti-modalFeedback', $choiceInteraction).length,
                        testCase.feedbacks.choice.length,
                        'modal feedbacks below choice interaction'
                    );
                    assert.equal(
                        $('.qti-modalFeedback', $orderInteraction).length,
                        testCase.feedbacks.order.length,
                        'modal feedbacks below order interaction'
                    );
                    assert.equal(
                        $('.qti-modalFeedback', $inlineInteractionContainer).length,
                        testCase.feedbacks.inline.length,
                        'modal feedbacks below inline block'
                    );
