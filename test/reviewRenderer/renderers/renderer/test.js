define([
    ...
    'taoQtiItem/reviewRenderer/renderers/Renderer',
    'json!taoQtiItem/test/samples/json/space-shuttle.json'
], function($, _, QtiLoader, reviewRenderer, itemData) {
    'use strict';
    ...
    QUnit.test('renders an item', function(assert) {
        ...
        var renderer = new reviewRenderer({ baseUrl: './' });
        ...
        new QtiLoader().loadItemData(itemData, function(item) {
            renderer.load(function() {
                ...
                assert.equal(
                    $('.qti-choiceInteraction', $result).length,
                    1,
                    'The result contains a choice interaction'
                );
    ...
    QUnit.test('renders decorated interactions', function(assert) {
        ...
        //Set up decorator
        var renderer = new reviewRenderer({
            baseUrl: './',
            decorators: {
                ...
                after: function postRender(element) {
                    if (element.qtiClass === 'choiceInteraction') {
        ...
        new QtiLoader().loadItemData(itemData, function(item) {
            renderer.load(function() {
                ...
                assert.ok(
                    $('.qti-choiceInteraction', $result)
                        .prev()
                        .hasClass('pre-choice'),
                    'The pre decorator is previous the choice interaction'
                );
                ...
                assert.ok(
                    $('.qti-choiceInteraction', $result)
                        .next()
                        .hasClass('post-choice'),
                    'The post decorator is next to the choice interaction'
                );
