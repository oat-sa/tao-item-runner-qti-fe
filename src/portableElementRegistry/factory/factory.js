...
export default function portableElementRegistry(methods) {
    var _loaded = false;
    var __providers = {};
            ...
            loadRuntimes: function loadRuntimes(options) {
                ...
                loadPromise = self.loadProviders(options).then(function(providers) {
                    ...
                    return Promise.all(loadStack).then(function(results) {
                        //TODO could remove one level of promise

                        var configLoadingStack = [];

                        //update registry
                        self._registry = _.reduce(
                            results,
                            function(acc, _pcis) {
                        ...
                        return Promise.all(configLoadingStack).then(function(moduleConfigs) {
                            var requireConfigAliases = _.reduce(
                                moduleConfigs,
                                function(acc, paths) {
                            ...
                            _requirejs(
                                ...
                                function() {
                                    ...
                                    _.each(arguments, function(creatorHook) {
                                        ...
                                        var i = _.findIndex(self._registry[id], {
                                            version: portableElementModel.version
                                        });
                                        if (i < 0) {
                                            self.trigger(
                                                'error',
                                                'no creator found for id/version ' +
                                                    id +
                                                    '/' +
                                                    portableElementModel.version
                                            );
                                        } else {
